...
const LiquidityLocked = () => {
  ...
  const lastCharAddress = () => {
    returnedTokensData &&
      isMyLockTokens &&
      returnedTokensData?.map((val, i) => {
        // if (val?.name === 'tokenAddress') {

        var oldAddress = val.tokenAddress.split("");
        var newAdd = [];
        for (var x = 0; x <= 4; x++) {
  ...
  const handleChange = async (event, value) => {
    ...
    setLoading(true);
    var pageNum = value;
    var tokensData = await getAllLiquidityTokensByChainID(pageNum);
  ...
  const handleChange2 = async (event, value) => {
    ...
    setLoading(true);
    var pageNum = value;
    const myLockedTokens = await getMyLockedLiquidity(pageNum);
          ...
          returnedTokensData.map((item, index) => {
            return (
              ...
              >
                ...
                >
                  <Link
                    to={`/liqudity/detail/${item.tokenAddress}`}
                    style={{
                      color: `${theme.palette.background.hoverColor}`,
                      textDecoration: "none",
                    }}
...
export default LiquidityLocked;
